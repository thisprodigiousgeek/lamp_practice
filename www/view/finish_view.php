    ...
    <?php if(count($carts) > 0){ ?>
      <table class="table table-bordered">
        ...
        <tbody>
          ...
          <tr>
            <td><img src="<?php print(h(IMAGE_PATH . $cart['image']));?>" class="item_image"></td><!-- エスケープ処理の実装 -->
            <td><?php print(h($cart['name'])); ?></td><!-- エスケープ処理の実装 -->
            <td><?php print(h(number_format($cart['price']))); ?>円</td><!-- エスケープ処理の実装 -->
            <td>
                <?php print(h($cart['amount'])); ?>個<!-- エスケープ処理の実装 -->
            </td>
            <td><?php print(number_format(h($cart['price'] * $cart['amount']))); ?>円</td><!-- エスケープ処理の実装 -->
          </tr>
          <?php } ?>
        </tbody>
      </table>
